...
当前 App 的实现会在设置页保存 `workspaceID` 时同步写入 Keychain，以确保运行时发现链路和手动配置保持一致；`authCookie` 仍只写 Keychain。
...
| E. Token-Cost-App 实现（方案 A） | 🟡 中 | 已实现 ✅ | ✅ 自动导入到 Keychain | ⭐ |
...
| 增加「自动从浏览器导入」按钮 | ✅ 已实现（v0.5.1）— 实现方案 A（`security` CLI + PBKDF2 + AES-CBC），含用户确认弹窗 |
